...
def process_upcoming():
    ...
    df['days_until_release'] = (pd.to_datetime(df['release_date']) - pd.Timestamp.today()).dt.days

    df_clean = df[[
        'id', 'title', 'release_date', 'release_year',
        'release_month', 'days_until_release', 'popularity',
        'vote_average', 'vote_count', 'genres', 'original_language', 'overview'
    ]]

    os.makedirs('../data/processed', exist_ok=True)
    df_clean.to_csv('../data/processed/upcoming_movies.csv', index=False)
    print(f" Sparade {len(df_clean)} kommande filmer till data/processed/upcoming_movies.csv")
